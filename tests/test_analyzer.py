...
def test_correlation_calculation(sample_config, small_sample_data):
    ...
    test_data["assessment_events"] = (
        test_data["event_category"] == "assessment"
    ).astype(int)
    ...
    if correlations.empty:
        print(
            "⚠️ Correlation matrix is empty - это может быть нормально для маленьких данных"
        )
        ...
        if correlations.shape[0] > 0 and correlations.shape[1] > 0:
            ...
            if len(valid_values) > 0:
                ...
                assert (valid_values >= -1.0001).all() and (
                    valid_values <= 1.0001
                ).all()
...
def test_student_clustering(sample_config, small_sample_data):
    ...
    if unique_students >= 2:  # Нужно хотя бы 2 студента для кластеризации
        clusters = analyzer.cluster_students(
            small_sample_data, n_clusters=min(2, unique_students)
        )
        ...
        print(
            f"⚠️ Not enough students for clustering: {unique_students} unique students"
        )
...
def test_empty_data_handling(sample_config):
    ...
    empty_data = pd.DataFrame(
        columns=["student_id", "event_time", "grade", "event_category"]
    )
    ...
    try:
        ...
        print(
            f"✅ Correlation handling: {'Empty DataFrame' if correlations.empty else 'Not empty'}"
        )
    except Exception as e:
