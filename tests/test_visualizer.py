...
def sample_config():
    ...
    return {
        'visualization': {
            ...
            'style': 'seaborn',
            'dpi': 100,
            'colors': ['#1f77b4', '#ff7f0e', '#2ca02c', '#d62728', '#9467bd', '#8c564b']  # Добавили больше цветов
        }
    }
...
def sample_grade_data():
    ...
    data = {
        ...
        'event_category': np.random.choice(['assessment', 'social', 'content_interaction', 'login'], 
                                          n_students)
    }
...
def test_correlation_heatmap(sample_config, sample_correlation_matrix, tmp_path):
    ...
    fig = visualizer.plot_correlation_heatmap(sample_correlation_matrix, save_path=str(save_path))
...
def test_student_clusters_visualization(sample_config, sample_cluster_data, tmp_path):
    ...
    # Создаем визуализацию
    result = visualizer.plot_student_clusters(sample_cluster_data, save_path=str(save_path))
...
def test_grade_distribution(sample_config, sample_grade_data):
    ...
    if 'grade' not in sample_grade_data.columns or sample_grade_data['grade'].isnull().all():
...
def test_html_report_generation(sample_config, tmp_path):
    ...
    analysis_results = {
        'basic_stats': {
            ...
            'event_distribution': {'assessment': 300, 'social': 200, 'content_interaction': 500}
        }
    }
