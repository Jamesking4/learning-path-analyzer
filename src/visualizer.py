...
class ResultVisualizer:
    ...
    def plot_correlation_heatmap(self, correlation_matrix, save_path=None):
        ...
        ax.set_title('Correlation Matrix of Learning Activities and Grades', 
    ...
    def plot_student_clusters(self, clusters_df, save_path=None):
        ...
        if n_students > 0:
            ...
            ax.set_title('Student Clusters by Learning Patterns', fontsize=14, fontweight='bold')
    ...
    def plot_activity_timeline(self, df, student_id=None, save_path=None):
        ...
        # Агрегация по дням
        daily_activity = student_data.groupby('date').size().reset_index(name='activity_count')
    ...
    def plot_grade_distribution(self, df, save_path=None):
        ...
        fig = make_subplots(
            rows=2, cols=2,
            subplot_titles=('Grade Distribution', 'Grade Box Plot', 
                          'Grade by Event Type', 'Cumulative Distribution'),
        ...
        fig.add_trace(
            go.Histogram(x=grades, name='Grades', nbinsx=20, 
        ...
        # Box plot
        fig.add_trace(
            go.Box(y=grades, name='Grades', marker_color=self.colors[1]),
            row=1, col=2
        )
        ...
        if 'event_category' in df.columns:
            ...
            fig.add_trace(
                go.Bar(x=grade_by_event['event_category'], 
        ...
        color_index = 3 if len(self.colors) > 3 else 0  # Используем первый цвет, если недостаточно цветов
        fig.add_trace(
            go.Scatter(x=sorted_grades, y=cumulative, mode='lines',
                    name='CDF', line=dict(color=self.colors[color_index], width=2)),
            row=2, col=2
        )
    ...
    def plot_student_comparison(self, df, student_ids, save_path=None):
        ...
        fig = make_subplots(
            ...
            subplot_titles=('Activity Timeline', 'Event Distribution',
                          'Grade Comparison', 'Activity Heatmap'),
        ...
        if 'event_category' in student_data.columns:
            event_dist = student_data.groupby(['student_id', 'event_category']).size().unstack(fill_value=0)
            
            for i, event_type in enumerate(event_dist.columns):
                fig.add_trace(
                    go.Bar(x=event_dist.index, y=event_dist[event_type],
                          name=event_type, marker_color=self.colors[i % len(self.colors)]),
                    row=1, col=2
                )
        
        # Сравнение оценок
        if 'grade' in student_data.columns:
            grade_comparison = student_data.groupby('student_id')['grade'].agg(['mean', 'count']).reset_index()
            
            fig.add_trace(
                go.Bar(x=grade_comparison['student_id'], y=grade_comparison['mean'],
                      name='Average Grade',
        ...
        if 'day_of_week' in student_data.columns and 'hour' in student_data.columns:
            heatmap_data = student_data.groupby(['day_of_week', 'hour']).size().unstack(fill_value=0)
    ...
    def generate_html_report(self, analysis_results=None, save_path='reports/analysis_report.html'):
