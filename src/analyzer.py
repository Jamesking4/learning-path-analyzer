...
class LearningAnalyzer:
    ...
    def calculate_basic_metrics(self, df):
        ...
        metrics = {
            ...
            'avg_events_per_student': len(df) / df['student_id'].nunique() if not df.empty and df['student_id'].nunique() > 0 else 0
        }
        
        # Обработка time_range для пустых данных
        if not df.empty and 'event_time' in df.columns and df['event_time'].notna().any():
            metrics['time_range'] = {
        ...
        if 'event_category' in df.columns and not df.empty:
            metrics['event_distribution'] = df['event_category'].value_counts().to_dict()
        else:
    ...
    def calculate_correlations(self, df):
        ...
        if 'grade_mean' in correlation_matrix.columns:
            grade_correlations = correlation_matrix['grade_mean'].sort_values(ascending=False)
    ...
    def _prepare_student_data(self, df):
        ...
        student_data.columns = ['_'.join(col).strip() for col in student_data.columns.values]
        ...
        # Нормализация данных
        numeric_cols = student_data.select_dtypes(include=[np.number]).columns
        student_data[numeric_cols] = self.scaler.fit_transform(student_data[numeric_cols])
    ...
    def cluster_students(self, df, n_clusters=None):
        ...
        feature_cols = [col for col in student_data.columns 
                       if col not in ['student_id'] and student_data[col].dtype in [np.float64, np.int64]]
    ...
    def analyze_time_patterns(self, df):
        ...
        if 'event_category' in df.columns:
            event_hourly = df.groupby(['hour', 'event_category']).size().unstack(fill_value=0)
    ...
    def _identify_successful_students(self, df, threshold=None):
        ...
        student_grades = df[df['grade'] > 0].groupby('student_id')['grade'].agg(['mean', 'count'])
