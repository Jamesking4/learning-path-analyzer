...
class RecommendationEngine:
    ...
    def _analyze_time_patterns(self, student_data):
        ...
        if "day_of_week" in student_data.columns:
            ...
            weekend_ratio = len(student_data[student_data["day_of_week"] >= 5]) / len(
                student_data
            )
        ...
        if "date" in student_data.columns:
            ...
            total_days = (
                student_data["date"].max() - student_data["date"].min()
            ).days + 1
    ...
    def _compare_with_successful_students(self, df, student_id):
        ...
        successful_freq = len(df[df["student_id"].isin(successful.index)]) / len(
            successful
        )
        ...
        if "event_category" in df.columns:
            student_social = len(
                student_data[student_data["event_category"] == "social"]
            )
            successful_social = len(
                df[
                    (df["student_id"].isin(successful.index))
    ...
    def _identify_successful_students(self, df):
        ...
        successful = student_grades[
            student_grades >= self.thresholds["min_grade_threshold"]
        ]
    ...
    def _analyze_overall_time_patterns(self, df):
        ...
        if "day_of_week" in df.columns:
            ...
            patterns["weekend_activity"] = (
                weekend_events / len(df) if len(df) > 0 else 0
            )
