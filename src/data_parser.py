...
class LogParser:
    ...
    def _extract_features(self, df):
        ...
        # Признаки активности
        df['is_assessment'] = df['event_type'].str.contains('assignment|quiz|exam', case=False)
        df['is_social'] = df['event_type'].str.contains('forum|comment|reply', case=False)
        df['is_content'] = df['event_type'].str.contains('view|download|read', case=False)
    ...
    def _categorize_events(self, df):
        ...
        def categorize_event(event_type):
            ...
            if any(login_event in event_lower for login_event in self.event_types['login_events']):
                return 'login'
            elif any(content_event in event_lower for content_event in self.event_types['content_events']):
                return 'content_interaction'
            elif any(assessment_event in event_lower for assessment_event in self.event_types['assessment_events']):
                return 'assessment'
            elif any(social_event in event_lower for social_event in self.event_types['social_events']):
                return 'social'
            elif any(important_event in event_lower for important_event in self.event_types['important_events']):
                return 'milestone'
            else:
                return 'other'
    ...
    def aggregate_student_data(self, df):
        ...
        student_stats = df.groupby('student_id').agg({
            ...
            'activity_duration': 'sum' if 'activity_duration' in df.columns else 'count'
        }).round(2)
        ...
        event_counts = df.groupby(['student_id', 'event_category']).size().unstack(fill_value=0)
